...
public abstract class HttpCallBackMeizi<T extends HttpResponseMeizi> implements Callback<T> {
    private String TAG = HttpCallBackMeizi.class.getSimpleName();
    ...
    public void onResponse(Call<T> call, Response<T> response) {
        if (response.isSuccessful()) {                                  //Http 状态码code:[200,300）
            ...
            try {
                HttpResponseMeizi errorResponse = gson.fromJson(errorBodyStr, HttpResponseMeizi.class);
                if (null != errorResponse) {
                    onFailure(-1, errorResponse.getError());
                } else {
